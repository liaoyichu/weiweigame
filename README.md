...
- **B747-8** - 经典四发宽体客机，双层驼背造型（程序化建模导出的真实 3D 模型）
- **A330-300** - 优雅双发宽体客机（程序化建模导出的真实 3D 模型）
- 所有机型按真实尺寸比例缩放（长度/翼展相对一致）

### 场景元素
- 🏙️ 海边城市与 CBD 高楼群
- 🏔️ 程序化山体（带碰撞检测）
- 🌊 海面与沙滩
- 🛣️ 机场跑道（11km，缩短版）与城市道路
- 🛫 机场设施：航站楼、登机廊桥、塔台、机库、跑道号码与进近灯
- ✈️ 来往飞机：停机坪停泊、跑道滑行与高空巡航（可相撞）
- 🏠 200+ 栋程序化建筑
- 🚗 100+ 辆移动车辆
- ☁️ 动态云层
...
├── build_models.js     # B747-8 / A330-300 GLB 模型生成脚本（Node + three.js）
├── lib/
...
│   ├── b747.glb        # B747-8 飞机模型
│   └── a330.glb        # A330-300 飞机模型
├── output/             # 自动化测试截图输出
└── progress.md         # 开发进度日志
```
